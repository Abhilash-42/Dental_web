...
//   ALLOWED_ORIGIN    the frontend's URL, e.g. https://drchandu.netlify.app
...
app.post("/api/appointments", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO appointments (name, phone, email, service, appt_date, appt_time, notes)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id, status`,
      [name, phone, email || null, service, date, time, notes || null]
    );
...
app.get("/api/appointments/status", async (req, res) => {
  ...
  if (!/^\d{10}$/.test(phone)) {
    return res.status(400).json({ error: "provide a 10-digit phone number" });
  }
  const result = await pool.query(
    `SELECT service, appt_date, appt_time, status FROM appointments
     WHERE phone = $1 ORDER BY created_at DESC LIMIT 3`,
    [phone]
  );
...
const KB = [
  { id: "hospital-info", keywords: ["address","location","where","hours","open","timing","time","phone","number","contact","call","doctor","chandu"],
  ...
  { id: "faq-paediatrics", keywords: ["child","kid","children","paediatric","pediatric"],
    text: "Paediatric dentistry covers check-ups, fillings and sealants for children in a calm, kid-friendly setting." },
  { id: "care-extraction", keywords: ["after extraction","post extraction","extraction care"],
  ...
  { id: "care-toothache", keywords: ["toothache","tooth pain","hurts"],
...
function retrieve(query, topK = 2) {
  ...
  const scored = KB.map((chunk) => {
    ...
    chunk.keywords.forEach((k) => {
      if (q.includes(k)) score += k.length > 5 ? 2 : 1;
    });
    return { chunk, score };
  }).filter((s) => s.score > 0);
...
async function askClaude(userQuery, contextChunks) {
  const system = `You are the assistant for Dr. Chandu's Multi-speciality Dental Hospital.
Answer briefly and warmly, using ONLY the context below. If the context doesn't cover the
question, say you're not sure and suggest calling 090522 09930.
...
${contextChunks.map((c) => "- " + c.text).join("\n")}`;
  ...
  const text = (data.content || [])
    ...
    .join("\n")
    .trim();
  return text || "I'm not sure about that one — please call 090522 09930 and our team can help.";
}

function detectIntent(query) {
  const q = query.toLowerCase();
  const wantsBooking = /\b(book|appointment|schedule|visit)\b/.test(q);
  const directService = [
    "root canal","implant","braces","whitening","cleaning","extraction",
    "paediatric","x-ray","veneer","crown","denture",
  ].find((t) => q.includes(t));
  const map = {
    "root canal": "Root Canals", implant: "Dental Implants", braces: "Braces",
    whitening: "Teeth Whitening", cleaning: "Teeth Cleaning", extraction: "Extractions",
    paediatric: "Paediatrics", "x-ray": "X-ray", veneer: "Veneers & Crowns",
    crown: "Veneers & Crowns", denture: "Dentures & Bridges",
  };
  return { wantsBooking, service: directService ? map[directService] : null };
}
...
app.post("/api/rag-chat", async (req, res) => {
  ...
  if (asksStatus || phoneMatch) {
    if (!phoneMatch) {
      return res.json({ text: "What's the 10-digit phone number you booked with? I'll check its status.", action: null });
    }
    const result = await pool.query(
      `SELECT service, appt_date, appt_time, status FROM appointments WHERE phone=$1 ORDER BY created_at DESC LIMIT 3`,
      [phoneMatch[0]]
    );
    if (result.rows.length === 0) {
      return res.json({
        text: `I don't see any appointment under ${phoneMatch[0]}. Want to book one now?`,
        action: { label: "Book an appointment now", service: null },
      });
    }
    const text = result.rows
      .map((r) => `${r.service} on ${new Date(r.appt_date).toDateString()} at ${r.appt_time} is ${r.status}.`)
      .join(" ");
    return res.json({ text, action: null });
  }
  ...
  const text =
    contextChunks.length > 0
      ? await askClaude(userQuery, contextChunks)
  ...
  const action =
    intent.wantsBooking || intent.service
      ? { label: intent.service ? `Book ${intent.service} now` : "Book an appointment now", service: intent.service }
      : null;
