...
function openAppointmentDetails(id){
  ...
  detailsContent.innerHTML = `
    ...
    ${appointment.reschedule_reason ? `
      <div class="details-notes reschedule-reason-display">
        <span>Latest reschedule reason</span>
        <p>${escapeHtml(appointment.reschedule_reason)}</p>
      </div>
    ` : ''}
...
async function loadRescheduleDoctors(currentDoctor=''){
  rescheduleDoctor.innerHTML = '<option value="">Loading doctors…</option>';
  rescheduleDoctor.disabled = true;

  try{
    const data = await apiFetch('/api/doctors');
    const doctors = Array.isArray(data.doctors) ? data.doctors : [];

    rescheduleDoctor.innerHTML = '<option value="">Select a doctor</option>';
    ...
    doctors.forEach(doctor => {
      ...
      option.textContent = doctor.specialization
        ...
        : doctor.name;
      rescheduleDoctor.appendChild(option);
    });

    rescheduleDoctor.disabled = doctors.length === 0;

    if(currentDoctor && doctors.some(doctor => doctor.name === currentDoctor)){
      rescheduleDoctor.value = currentDoctor;
    }
  }catch(err){
    console.error('Could not load doctors for rescheduling:', err);
    rescheduleDoctor.innerHTML = '<option value="">Could not load doctors</option>';
    rescheduleDoctor.disabled = true;
    showRescheduleMessage(err.message || 'Could not load doctors.');
  }
}
...
async function loadRescheduleSlots(){
  ...
  try{
    const selectedDoctor = rescheduleDoctor.value;
    if(!selectedDoctor){
      rescheduleTime.innerHTML = '<option value="">Select a doctor first</option>';
      rescheduleSlotHint.textContent = 'Choose a doctor to see available slots.';
      return;
    }

    const query = `?doctor=${encodeURIComponent(selectedDoctor)}&date=${encodeURIComponent(rescheduleDate.value)}`;
    ...
    setRescheduleTimeOptions(
      data.slots || [],
      selectedDoctor === reschedulingAppointment.doctor && normalizeAppointmentDate(reschedulingAppointment.appt_date) === rescheduleDate.value
        ? reschedulingAppointment.appt_time
        : ''
    );

    const count = (data.slots || []).filter(slot => slot.available).length;
    rescheduleSlotHint.textContent = count
      ? `${count} available slot${count === 1 ? '' : 's'} for ${selectedDoctor}.`
...
async function openReschedule(id){
  ...
  await loadRescheduleDoctors(appointment.doctor || '');
  rescheduleTime.innerHTML = '<option value="">Loading available times…</option>';
  rescheduleTime.disabled = true;
...
async function submitReschedule(){
  ...
  const doctor = rescheduleDoctor.value;
  const date = rescheduleDate.value;
  const time = rescheduleTime.value;
  const reason = rescheduleReason.value.trim();

  if(!doctor || !date || !time){
    showRescheduleMessage('Please select a doctor, new date, and available time.');
    return;
  }

  if(!reason){
    showRescheduleMessage('Please provide a reason for rescheduling.');
    return;
  }
...
rescheduleDoctor.addEventListener('change', loadRescheduleSlots);
